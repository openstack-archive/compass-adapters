...
template File.join(node['nginx']['dir'], 'sites-available', 'nagios4.conf') do
  ...
  if File.symlink?(File.join(node['nginx']['dir'], 'sites-enabled', 'nagios4.conf'))
    notifies :reload, 'service[nginx]', :immediately
  end
end

nginx_site 'nagios4.conf' do
  notifies :reload, 'service[nginx]'
end
