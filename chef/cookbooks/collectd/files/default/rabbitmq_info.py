...
def get_stats():
    ...
    try:
        p = subprocess.Popen(['curl', '-s', '-i', '-u', USERPASS, RABBITMQ_API],
...
#        stats['ctl_consumers_%s' % queue_name] = ctl_stats[3]
    ...
    for i in range(0, len(resp)-1):
        ...
        stats['ctl_memory_%s' % resp[i]['memory']] = resp[i]['memory']
        stats['ctl_consumers_%s' % resp[i]['consumers']] = resp[i]['consumers']
