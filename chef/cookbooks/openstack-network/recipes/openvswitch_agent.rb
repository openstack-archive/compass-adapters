...
if node['platform_family'] == 'suse'
  if node['lsb']['codename'] == 'UVP'
    template '/etc/init.d/openstack-neutron-openvswitch-agent' do
      source 'openstack-neutron-openvswitch-agent.service.erb'
      owner "root"
      group "root"
      mode 00755
    end
...
ruby_block "service neutron-plugin-openvswitch-agent restart if necessary" do
  block do
    Chef::Log.info("service neutron-plugin-openvswitch-agent restart")
  end
  not_if "service #{platform_options['neutron_openvswitch_agent_service']} status"
  notifies :restart, 'service[neutron-plugin-openvswitch-agent]', :immediately
end
