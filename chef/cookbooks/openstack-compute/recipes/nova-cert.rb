...
if node['platform_family'] == 'suse'
  if node['lsb']['codename'] == 'UVP'
    template '/etc/init.d/openstack-nova-cert' do
      source 'openstack-nova-cert.service.erb'
      owner "root"
      group "root"
      mode 00755
    end
...
ruby_block "service nova-cert restart if necessary" do
  block do
    Chef::Log.info("service nova-cert restart")
  end
  not_if "service #{platform_options['compute_cert_service']} status"
